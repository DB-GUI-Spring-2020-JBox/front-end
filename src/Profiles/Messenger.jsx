...
export class Messenger extends React.Component {
  ...
  onEdit(index) {
    this.setState({
      editMessage: index,
      currentMessage: this.state.messages[index].content,
    });
  }
  ...
  onSwitchMessages(id, name) {
    ...
    let messages = messageList.filter(
      (x) =>
        (x.sender === sender && x.recipient === id) ||
        (x.sender === id && x.recipient === sender)
    );
  ...
  componentWillMount() {
    ...
    messageList.forEach((msg) => {
      if (
        msg.sender === sender &&
        !uniqueUsers.find((x) => x.id === msg.recipient)
      ) {
        uniqueUsers.push({
          id: msg.recipient,
          name: userList.find((x) => x.id === msg.recipient).name,
        });
      }
    });
    ...
    let messages = messageList.filter(
      (x) =>
        (x.sender === sender && x.recipient === uniqueUsers[0].id) ||
        (x.sender === uniqueUsers[0].id && x.recipient === sender)
    );
    messages.sort((a, b) => a.datePosted - b.datePosted);
  ...
  render() {
    ...
    return (
      <div id="messenger" className="container row">
        ...
        <div
          id="message-section"
          className={"container " + (isPhone ? "" : "col-9")}
        >
          <section id="messenger-header" className="row">
            <h2 className={isPhone ? "" : "col-8"}>
              {this.state.recipientName}
            </h2>
            <div className={"pt-1 " + (isPhone ? "col-12" : "col-4")}>
              <Link
                className={
                  "btn btn-warning " + (isPhone ? "btn-block" : "float-right")
                }
  ...
  componentDidMount() {
    ...
    if (recipientId !== this.state.recipient) {
      this.onSwitchMessages(
        recipientId,
        userList.find((x) => x.id === recipientId)
      );
    }
